...
const words = ["intelligent", "smarter", "faster", "automated", "secured"];
const systemWords = ["AI", "cloud", "digital", "modern", "intelligent"];
...
export function HeroSection() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSystemIndex((prev) => (prev + 1) % systemWords.length);
    }, 3000);
    return () => clearInterval(interval);
  }, []);
            ...
            <span className="block">
              <span className="relative inline-block">
                <span 
                  ...
                  className="inline-flex"
                >
                  {systemWords[systemIndex].split("").map((char, i) => (
                    <span
                      key={`${systemIndex}-${i}`}
                      className="inline-block animate-char-in"
                      style={{
