...
import { ShieldCheck, KeyRound, ScrollText, Lock, CloudCog, Code2, BadgeCheck, Fingerprint, ArrowRight } from "lucide-react"

const cards = [
  { icon: ShieldCheck, label: "Role-Based Access Control" },
  { icon: KeyRound, label: "Data Encryption" },
  { icon: ScrollText, label: "Audit Logs" },
  { icon: Lock, label: "Private AI Deployment" },
  { icon: CloudCog, label: "Cloud or On-Premise" },
  { icon: Code2, label: "API Security" },
  { icon: BadgeCheck, label: "Compliance Ready" },
  { icon: Fingerprint, label: "Identity Management" },
]
      ...
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-14">
          ...
          >
            ...
            <span className="text-muted-foreground text-sm">Secure University Cloud</span>
          </motion.div>
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          {cards.map((card, i) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -3 }}
                className="flex flex-col items-center text-center gap-3 rounded-2xl border border-border/60 hover:border-indigo-500/40 bg-white/50 dark:bg-zinc-900/40 backdrop-blur-sm p-5 transition-colors"
              >
